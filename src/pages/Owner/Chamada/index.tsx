...
export default function OwnerChamada() {
  const { turmaId = '' } = useParams(); const { placeId, procurando } = useEspacoDoEndereco({ tipo: 'turma', id: turmaId })
  ...
  return <div><Voltar onClick={() => navigate(`/owner/turmas?placeId=${placeId}`)}><ArrowLeft size={16}/>Voltar para as turmas</Voltar>
    <Caixa><Topo><div><Titulo>Aulas da turma</Titulo><Ajuda>Aulas dadas continuam aqui, para a chamada poder ser corrigida.</Ajuda></div><label>Dia <Data aria-label="Dia" type="date" value={dia} onChange={e=>{setDia(e.target.value);setAulaId('')}}/></label></Topo>
      {/* Ver o comentário em `Owner/Turmas`: consulta desabilitada fica
          `isPending` para sempre. Sem o `!placeId` primeiro, quem abre a URL
          sem o espaço lia "Nenhuma aula neste dia" — que é pior que o
          esqueleto, porque afirma algo falso. Sem `?placeId=`, o espaço é
          procurado nos do dono (web#520). */}
      {procurando ? <Estado>Procurando o espaço da turma…</Estado> : !placeId || !turmaId ? <Estado role="alert">Não achamos esta turma nos seus espaços. Volte para Turmas e abra a turma por lá.</Estado> : aulas.isPending ? <Estado>Carregando aulas…</Estado> : aulas.data?.length ? <Lista>{aulas.data.map(a => <Linha key={a.id}><span>{hora(a.inicio)}–{hora(a.fim)} · {a.status.toLowerCase()}</span><Botao onClick={()=>setAulaId(a.id)} disabled={a.status==='CANCELADA'}>{a.status==='DADA'?'Corrigir chamada':'Abrir chamada'}</Botao></Linha>)}</Lista> : <Estado>Nenhuma aula neste dia.</Estado>}
    </Caixa>
