...
import {
  Acoes, Ajuda, Atencao, Aviso, Botao, BotaoRegistrar, Campo, Cancelar, Confirmar, Dados, Data,
  Detalhe, Estado, Grupo, Linha, Lista, ModalAcoes, ModalCaixa, ModalFundo, ModalTexto, ModalTitulo,
  Nome, Selecao, Selo, Selos, Topo, Travado,
} from './styles'
...
function situacao(assinatura: AssinaturaDoAdmin): { tom: TomDeSelo; rotulo: string } {
  ...
  if (!assinatura.emDia) return { tom: 'erro', rotulo: 'Vencida' }
 ...
 *
 * Só a manual entra: a da Stripe tem webhook, e um cartão recusado lá vira
      ...
      {atencao.length > 0 && (
        <Atencao role="status">
          ...
          <span>
            <strong>
              {atencao.length === 1
                ? '1 assinatura manual precisa de atenção'
                : `${atencao.length} assinaturas manuais precisam de atenção`}
            </strong>
            {': '}
            {atencao.map((a) => a.owner.name).join(', ')}. Assinatura manual não se renova
            sozinha — vencida, o dono perde o acesso sem ninguém ser avisado.
          </span>
        </Atencao>
      )}
                ...
                <Dados>
                  ...
                  <Detalhe>
                    ...
                    {assinatura.currentPeriodEnd
                      ...
                      : manual
                        /* Nunca vazio numa manual: sem data ela não vale acesso
                           nenhum, e um traço no lugar esconderia justamente o
                  ...
                  {assinatura.registradaPor && (
                    <Detalhe>
                      Registrada por {assinatura.registradaPor}
                      {assinatura.registradaEm && ` em ${dia(assinatura.registradaEm)}`}
                    </Detalhe>
                  )}
                  <Selos>
                    <Selo $tom={manual ? 'manual' : 'stripe'}>
                      {manual ? 'Manual (Pix)' : 'Stripe'}
                    </Selo>
